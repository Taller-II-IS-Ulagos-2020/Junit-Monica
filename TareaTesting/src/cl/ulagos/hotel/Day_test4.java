...
public class Day_test4
{
    ...
    private Appointment_test4[] appointments;

    /**
     * Constructor for objects of class Day.
     * @param dayNumber The number of this day in the year (1-366).
     */
    public Day_test4(int dayNumber)
    {
        this.dayNumber = dayNumber;
        appointments = new Appointment_test4[MAX_APPOINTMENTS_PER_DAY];
    }
    ...
    public boolean makeAppointment(double time,
                                   Appointment_test4 appointment)
    {
        if(validTime(time)) {
        	double startTimeaux= ((time - START_OF_DAY)*2);
        	int startTime = (int) Math.round(startTimeaux);
            ...
            if(appointments[startTime] == null) {
                ...
                // of the appointment.
                int durationaux = (int)Math.round(duration*2);
                for(int i = 0; i < durationaux; i++) {
     ...
     */
    public Appointment_test4 getAppointment(double time)
    {
        if(validTime(time)) {
        	int aux = (int)Math.round((time - START_OF_DAY)*2);
            return appointments[aux];
        }
        else {
            return null;
        }
    ...
    public void showAppointments()
    {
        ...
        for(Appointment_test4 appointment : appointments) {
